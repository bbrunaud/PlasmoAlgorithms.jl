#use exact the same formulation and bounds as Kannan's code
include("input.jl")
function generate_model()
	...
	@variable(m, 0<=campaignDuration[n in events, h in scenarios]<= sum(ProductionLength_upper))
	@variable(m, 0<=amtProductInCampaign[p in products, n in events, h in scenarios]<=sum(ProductionLength_upper) * MaxProductionRate[p])
	...
	@variable(m, 0<=cycleTime[h in scenarios]<=length(events)*sum(ProductionLength_upper)+sum(CampaignSetupTime))
