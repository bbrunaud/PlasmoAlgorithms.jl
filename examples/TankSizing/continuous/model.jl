...
function generate_model()
	...
	@variable(m, 0<=amtProductInCampaign[p in products, n in events, h in scenarios]<= ProductionLength_upper[p] * MaxProductionRate[p])
	...
	@variable(m, 0<=auxiliaryVariable[p in products, n in events, h in scenarios]<=(InventoryUpperBound[p] - InventoryLowerBound[p])/30)
	...
	@variable(m, minimum(ProductionLength_lower + CampaignSetupTime)<=campaignLength[n in events, h in scenarios]<= maximum(ProductionLength_upper + CampaignSetupTime)) 
	...
	@variable(m, slack[p in products, h in scenarios]>=0)
	#constraints
	...
	@constraint(m, TankCapacity[p in products, n in events, h in scenarios], productInventory[p,n,h] <= productTankSize[p] + slack[p,h])
	@constraint(m, ProductionUpperBound1[p in products, n in events, h in scenarios], amtProductInCampaign[p,n,h] <= MaxProductionRate[p] * ProductionLength_upper[p] * assignProductToCampaign[p,n,h])
	@constraint(m, ProductionLowerBound1[p in products, n in events, h in scenarios], amtProductInCampaign[p,n,h] >= MinProductionRate[p] * ProductionLength_lower[p] * assignProductToCampaign[p,n,h])
	@constraint(m, ProductionUpperBound2[p in products, n in events, h in scenarios], amtProductInCampaign[p,n,h] <= MaxProductionRate[p] * campaignDuration[n,h])
	@constraint(m, ProductionLowerBound2[p in products, n in events, h in scenarios], amtProductInCampaign[p,n,h] >= MinProductionRate[p] * (campaignDuration[n,h] - ProductionLength_upper[p] * (1 - assignProductToCampaign[p,n,h])))
	...
	@constraint(m, CampanLengthDef[n in events, h in scenarios], campaignLength[n,h] == campaignDuration[n,h] + sum(CampaignSetupTime[pp] * assignProductToCampaign[pp,n,h] for pp in products))
	...
	@NLconstraint(m, CampaignStorageCost[h in scenarios], variableCost[h] == sum(CampaignVariableCost[p] * auxiliaryVariable[p,n,h] * campaignLength[n,h] for p in products for n in events ))
	...
	#to do 
	@NLconstraint(m, CampaignCostPerTon[h in scenarios], costPerTon[h]*cycleTime[h]*TotalDemandPerDay[h] == setupCost[h] + variableCost[h])
	...
	@NLobjective(m, Min, sum(prob[h] * costPerTon[h] + prob[h] * sum(slack[p,h] for p in products) for h in scenarios) + VariableInvestmentCostFactor * sum(prob[h] /TotalDemandPerDay[h] for h in scenarios) *sum(productTankSize[p]^0.5 for p in products) )
