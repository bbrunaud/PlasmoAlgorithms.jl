function generate_boundsub(;prob = prob[1], DemandPerDay = DemandPerDay[:, 1], TotalDemandPerDay=TotalDemandPerDay[1])
	m = Model(solver=BaronSolver(maxtime=5e4, epsr= 1e-4, prlevel=0, CplexLibName = "/opt/ibm/ILOG/CPLEX_Studio127/cplex/bin/x86-64_linux/libcplex1270.so"))
	...
	@variable(m, 0<=auxiliaryVariable[p in products, n in events]<=InventoryUpperBound[p] - InventoryLowerBound[p])
	@variable(m, investmentCost)
	@variable(m, setupCost)
	@variable(m, variableCost)
	@variable(m, 0<=cycleTime<=length(events)*maximum(ProductionLength_upper)+sum(CampaignSetupTime))
	@variable(m, costPerTon)
