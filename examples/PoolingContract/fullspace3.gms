...
loop(ww1,
	num = ord(ww1);
	prob(w2)$(ord(w2)=num) = base_prob(ww1); 
	DU(j, w2)$(ord(w2)=num) = base_DU(j) * ratio(ww1);
	d(j,w2)$(ord(w2)=num) = base_d(j);
	psi_f(i,w2)$(ord(w2)=num) = base_psi_f(i) ;
	psi_d2(i,w2)$(ord(w2)=num) = base_psi_d2(i) ;
	psi_d1(i,w2)$(ord(w2)=num) = base_psi_d1(i)  ;
	psi_b1(i,w2)$(ord(w2)=num) = base_psi_b1(i)  ; 
	psi_b2(i,w2)$(ord(w2)=num) = base_psi_b2(i) 	;
