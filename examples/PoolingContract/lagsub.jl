function generate_lagsub(;psi_f=base_psi_f, psi_d1=base_psi_d1, psi_d2=base_psi_d2, psi_b1=base_psi_b1,d = base_d,  psi_b2=base_psi_b2, DU=base_DU, prob=0)
	...
	@objective(m, Min,  prob * (sum(c_fixed_inlt[i] * gamma_intlt[i]+c_variable_inlt[i]*A[i] for i in feeds) + sum(c_fixed_pool[l] * gamma_pool[l] + c_variable_pool[l]*S[l] for l in pools) )+ prob*(sum(CT[i] for i in feeds) -sum(d[j]*(sum(y[l,j] for l in pools if (l,j) in Ty) + sum(z[i,j] for i in feeds if (i,j) in Tz)) for j in products) ) )
