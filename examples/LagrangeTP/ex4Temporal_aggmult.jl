...
g1 = deepcopy(g)
@linkconstraint(g1, [i in oproducts, t in 1:otime[end-1]], sum(node[t][:vf][s,i,t] for s in sites)  == sum(node[t+1][:vi][s,i,t+1] for s in sites))
push!(G,g1)

#Aggregation of multipliers - Product
g2 = deepcopy(g)
@linkconstraint(g2, [s in sites, t in 1:otime[end-1]], sum(node[t][:vf][s,i,t] for i in oproducts) == sum(node[t+1][:vi][s,i,t+1] for i in oproducts))
push!(G,g2)
