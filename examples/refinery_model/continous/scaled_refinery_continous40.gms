...
*-------------------------------------
	...
	CDU_capacity_bound .. 	sum(c, crudeQuantity(c)) =l= CDU_capacity
;

	Crude_selection(c) .. crudeQuantity(c) =g= pickCrude(c)*Crude_lower_bound(c)
;

	Crude_bound(c) .. 	CrudeQuantity(c) =l= pickCrude(c)*Crude_upper_bound(c)
;
...
;
	...
	Mass_balance1(h) .. Reformer_fraction('1','1')*flow_Reformer95(h) +
						...
						flow_Burn('1',h) +
						sum(c,
							Crude_yield_data(c,'1',h)*crudeQuantity(c) +
...
;
	...
	Mass_balance2(h) .. Reformer_fraction('1','2')*flow_Reformer95(h) +
						...
						sum(c,
							Crude_yield_data(c,'2',h)*crudeQuantity(c)
						) =e= 0
;
	...
	Mass_balance3(h) .. -flow_LN_producing(h) - flow_Burn('3',h) -
						...
						sum(c,
							Crude_yield_data(c,'3',h)*crudeQuantity(c)
						) =e= 0
;
	...
	Mass_balance4(h) .. -flow_JPF('1',h)*JPF_fraction('2','1') -
						...
						sum(c,
							Crude_yield_data(c,'4',h)*crudeQuantity(c)
						) =e= 0
;
	...
	Mass_balance5(h) .. -flow_JPF('1',h)*JPF_fraction('3','1') -
						...
						flow_AGO_3('1',h) +
						sum(c,
							Crude_yield_data(c,'5',h)*crudeQuantity(c)
						) =e= 0
;

	Mass_balance7(h) .. -flow_Cracker_Mogas(h) - flow_Cracker_AGO(h) +
						sum(c,
							Crude_yield_data(c,'7',h)*crudeQuantity(c)
						) =e= 0
;

	GO_balance(c,h) .. -flow_AGO_1(c,h) - flow_Desulphurisation_1(c,h) - flow_HF_3(c,h) +
						Crude_yield_data(c,'6',h)*crudeQuantity(c) =e= 0
;

	VR_balance(c,h) .. 	Crude_yield_data(c,'8',h)*crudeQuantity(c) =e= flow_HF_1(c,h)
;
...
;
    ...
    objfn .. objvar =e= sum(h,prob(h)*(
								...
								sum(c,
									...
									(crudeQuantity(c)/1000/BarrelToKT(c)*GranularityOfBarrels)*(Crude_price(c)+1)
								) -
								sum(PG98_in,
